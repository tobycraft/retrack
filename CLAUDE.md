...
6. **Investigated: intercepting the native Ctrl+V paste / Paste Options gallery directly.** Confirmed via Microsoft's own docs that neither is possible from a JS Office Add-in: Ctrl+X/C/V are explicitly on the reserved-shortcuts list custom keyboard shortcuts cannot rebind, and the Word JS API has no pre-edit/cancelable paste event at all (`onParagraphChanged` etc. only fire *after* an edit, don't distinguish paste from typing). Also confirmed the native Paste Options gallery (Keep Source Formatting / Merge Formatting / Keep Text Only / etc.) can't be extended by *any* add-in technology, including the older, more privileged RibbonX/VSTO model — built-in galleries can be hidden or have their action overridden wholesale, never appended to. True interception only exists via VBA (`Application.OnKey "^v", "MyMacro"`), which is the same desktop-only/non-distributable dead end as decision #3. Two things did come out of this as real candidates: (a) a `ContextMenu` extension point — a custom "Apply Minimal Paste" entry on the native right-click menu, the closest available surface to the Paste Options gallery, built alongside the ribbon button in `manifest.xml` (same `applyMinimalPaste` function, same clipboard-read risk as #5); (b) a background auto-collapse design using Word's "event-based activation" (`OnDocumentOpened`, a long-lived shared runtime, then `onParagraphChanged`) to detect a freshly-pasted adjacent `<w:del>`/`<w:ins>` pair and silently collapse it after a normal Ctrl+V — not yet prototyped; carries heuristic/re-entrancy risk and Microsoft currently lists Mac support for the activation event as "coming later."
